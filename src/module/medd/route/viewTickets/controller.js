...
angularApp.controller('support', function($scope, $rootScope, $http, $localStorage, $timeout, $interval, $window, $filter, $location, restApi, Tickets){
	...
	function fetchUserData(){
		if($localStorage.admin_token){
			...
			$http.get('../server/db_transactions/resources.php', {params:{"requested_resource": "all", "security_token": $scope.admin_token}}).then(function successCallback(response){
			...
			$http.get('../server/db_transactions/isClient.php').then(function successCallback(response){
	...
	$scope.saveModalData = function(type){
		...
		var typeIndex = types.indexOf(type);
		$http.post('../server/db_transactions/'+updateDir[typeIndex], data[typeIndex]).then(function successCallback(response){
	...
	$scope.eliminarDatos = function(type, data){
		...
		if($window.confirm('¿Estas seguro?')){
			$('#preloaderScreen').modal('open');
			$http.post('../server/db_transactions/'+dirs[typeIndex], data).then(function successCallback(response){
				if(response.data.status == "success"){
	...
	$scope.formSubmit = function(nom_formulario){
		...
		$http.post('../server/db_transactions/'+addressToSend[mainIndex], objectToSend[mainIndex]).then(function successCallback(response){
			if(response.data.status === "success"){
