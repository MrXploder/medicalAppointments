    ...
    .directive('contextMenu', ['$rootScope', 'ContextMenuEvents', '$parse', '$q', 'CustomService', '$sce', '$document', '$window',
      function($rootScope, ContextMenuEvents, $parse, $q, custom, $sce, $document, $window) {
        ...
        function initContextMenuContainer(params) {
          ...
          var $ul = $('<ul>');
          $ul.addClass('dropdown-menu card');
          $ul.attr({
            'role': 'menu'
          });
          $ul.css({
        ...
        function removeContextMenus(level) {
          while (_contextMenus.length && (!level || _contextMenus.length > level)) {
            ...
            $rootScope.$broadcast(ContextMenuEvents.ContextMenuClosed, {
              context: _clickedElement,
              contextMenu: cm
            });
            cm.remove();
          ...
          if (!level) {
            $rootScope.$broadcast(ContextMenuEvents.ContextMenuAllClosed, {
              context: _clickedElement
            });
          }
        }
            ...
            element.on(openMenuEvent.trim(), function(event) {
              ...
              $scope.$apply(function() {
                ...
                $q.when(options).then(function(promisedMenu) {
                  ...
                  $rootScope.$broadcast(ContextMenuEvents.ContextMenuOpening, {
                    context: _clickedElement
                  });
